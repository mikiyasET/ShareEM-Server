...
userRoute.put('/complete-reg', multer().none(), async (req: any, res) => {
    if (req.user.status == STATUS.inactive) {
        ...
        const {anonymous, username, fName, lName, gender} = req.body;
        if (anonymous === "false") {
            ...
            if (anonymous === undefined) {
                return res.status(400).json({
                    success: false,
                    data: null,
                    message: "REQUIRED_FIELDS_MISSING2"
                });
            }
            return updateUser(req.user.id, {
                status: STATUS.active
                ...
                return res.status(200).json({
                    success: true,
                    data: user,
                    message: "REGISTRATION_COMPLETED2"
                });
            }).catch((err) => {
                return res.status(400).json({
                    success: false,
                    data: null,
                    message: "REGISTRATION_COMPLETION_FAILED2"
                });
            });
        }
    } else {
